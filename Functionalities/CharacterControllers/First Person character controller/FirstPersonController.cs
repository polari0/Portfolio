...
//
//This version of the Code was made by Polari
//
// Edited the entire code to fit the New unity Input system had to change lot of the hold to do X features to toggles
// Most of the told code is still here just commented out if you are interested || Version 1.0.2 
...
public class FirstPersonController : MonoBehaviour
{
    ...
    #region different Movement actions and what they do
    ...
    {
        //with how the original assets was set up and how the new input system handles the Hold action this is much easier to just be left at toggle
        if (enableCrouch)
        {
            if (!holdToCrouch)
            {
    ...
    {
        //Sprint will also be left as toggle instead of hold down action as new input system seems to disagree with it being possibility. 
        if(!sprintToggle)
        {
            Debug.Log("We sprint");
            sprintToggle = true;
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            else if (enableSprint && !sprintToggle)
            {
                isSprinting = false;
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Label("Originally by Jess Case", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Normal, fontSize = 12 });
        GUILayout.Label("Adabted for new input system by Polari", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Normal, fontSize = 12 });
